    ...
    if (products.length > 0) {
        ...
        if (found === undefined) {
            ...
            if (!status)
                ...
                return new statusMessage.Status("added product successfully");
        } else
    ...
    if (products.length > 0) {
        if (products.find((p) => p.productId === pid) !== undefined) {
            ...
            if (!status)
                ...
                return new statusMessage.Status("removed product successfully");
        } else
...
function updateProduct(product) {
    ...
    if (products.length > 0) {
        ...
        if (found) {
            products.splice(products.indexOf(found), 1, product);
            let status = productDao.write(products);
            if (!status)
                return new statusMessage.Status("couldn't update product");
            else
                return new statusMessage.Status("updated product successfully");
